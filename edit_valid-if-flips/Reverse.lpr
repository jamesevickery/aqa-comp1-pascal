...
Function CheckWhetherOrNotTheMoveWillResultInAtLeastOnePieceBeingFlipped(Board : TBoard;
  BoardSize, Move : Integer; HumanPlayersTurn : Boolean) : Boolean;
  Var
    Row : Integer;
    Column : Integer;
    WhetherThereAreFlipsInTheDifferentDirections : Array [1..4] Of Boolean;
    TheBoardWeWillTryOutTheMoveOn : TBoard;
  Begin
    Row := Move Mod 10;
    Column := Move Div 10;

    TheBoardWeWillTryOutTheMoveOn := Board;

    If HumanPlayersTurn
      Then TheBoardWeWIllTryOutTheMoveOn[Row, Column] := 'H'
      Else TheBoardWeWIllTryOutTheMoveOn[Row, Column] := 'C';

    WhetherThereAreFlipsInTheDifferentDirections[1] :=
        CheckIfThereArePiecesToFlip(TheBoardWeWillTryOutTheMoveOn, BoardSize, Row, Column, 1, 0);
    WhetherThereAreFlipsInTheDifferentDirections[2] :=
        CheckIfThereArePiecesToFlip(TheBoardWeWillTryOutTheMoveOn, BoardSize, Row, Column, -1, 0);
    WhetherThereAreFlipsInTheDifferentDirections[3] :=
        CheckIfThereArePiecesToFlip(TheBoardWeWillTryOutTheMoveOn, BoardSize, Row, Column, 0, 1);
    WhetherThereAreFlipsInTheDifferentDirections[4] :=
        CheckIfThereArePiecesToFlip(TheBoardWeWillTryOutTheMoveOn, BoardSize, Row, Column, 0, -1);

    CheckWhetherOrNotTheMoveWillResultInAtLeastOnePieceBeingFlipped :=
        WhetherThereAreFlipsInTheDifferentDirections[1] Or
        WhetherThereAreFlipsInTheDifferentDirections[2] Or
        WhetherThereAreFlipsInTheDifferentDirections[3] Or
        WhetherThereAreFlipsInTheDifferentDirections[4];
  End;
    ...
    Begin
      ...
      Repeat
        ...
        Repeat
          ...
          MoveIsValid := CheckIfMoveIsValid(Board, Move)And
              CheckWhetherOrNotTheMoveWillResultInAtLeastOnePieceBeingFlipped(Board, BoardSize, Move,
              HumanPlayersTurn);
        Until MoveIsValid;
